...
export async function listarRequisicoesPorColaborador(usuarioId) {
    let retorno;
    await getDocs(collection(db, "requisicoes"))
        .then((querySnapshot) => {
            retorno = querySnapshot.docs.map((doc) => ({ ...doc.data(), id: doc.id })).filter(doc => usuarioId === doc.usuarioId);
        });
    return retorno;
}
